...
public static class PlayerNumberingExtensions
{
    ...
    public static void SetPlayerNumber(this Player player, int playerNumber)
    {
        ...
        if (playerNumber < 0)
        {
            Debug.LogWarning("Configuración de un número de jugador no válido: " + playerNumber + " por: " + player.ToStringFull());
        }

        if (!PhotonNetwork.IsConnectedAndReady)
        {
            Debug.LogWarning("SetPlayerNumber fue llamado en estado: " + PhotonNetwork.NetworkClientState + ". No está conectado y listo.");
            return;
        }
        ...
        {
            Debug.Log("Numeración del jugador: Establecer número " + playerNumber);
